...
import io.kestra.scheduler.stores.InMemoryFlowMetaStore;
import io.kestra.scheduler.stores.InMemoryTriggerStateStore;
...
class TriggerEventHandlerTest {
    ...
    void shouldCompleteTriggerGivenTriggerCompletedEventWhenHandled() {
        ...
        triggerStateStore.save(triggerState);
        handler = newTriggerEventHandler(List.of());
        TriggerCompleted event = new TriggerCompleted(triggerId, IdUtils.create(), State.Type.SUCCESS);
    ...
    void shouldExecuteTriggerGivenFlowAndExecutionWhenHandled() {
        ...
        triggerStateStore.save(triggerState);
        handler = newTriggerEventHandler(List.of(Fixtures.defaultFlow()));
        TriggerExecuted event = new TriggerExecuted(triggerId, Execution.builder()
            .id(IdUtils.create())
            .state(new State())
            .build()
        );
