 ...
 * NOTE: it uses the <code>defaultFilter(tenantId)</code> for querying. If the child repository uses a default filter, it should override it.
 *
 * @param <T> the type of the persisted entity.
 */
public class AbstractJdbcCrudRepository<T> extends AbstractJdbcRepository {
    ...
    /**
     * Save a list of items: persist them inside the database and return udpated count.
     */
        ...
        return this.jdbcRepository
            ...
            .transactionResult(configuration -> {
                DSL.using(configuration)
                    ...
                    .where(field("key").eq(queueService.key(current)))
                    .execute();
     ...
     */
    public Optional<T> findOne(String tenantId, Condition condition) {
        return this.jdbcRepository
            .getDslContextWrapper()
            .transactionResult(configuration -> {
                var select = DSL
     ...
     *
     * @see #findAsync(String, Condition)
     * @see #findPage(Pageable, String, Condition)
     */
    public List<T> find(String tenantId, Condition condition) {
        return this.jdbcRepository
            .getDslContextWrapper()
            .transactionResult(configuration -> {
                var select = DSL
     ...
     *
     * @see #find(String, Condition)
     * @see #findPage(Pageable, String, Condition)
     */
    public Flux<T> findAsync(String tenantId, Condition condition) {
     ...
     * @see #findAsync(String, Condition)
     */
    public ArrayListTotal<T> findPage(
        Pageable pageable,
        String tenantId,
        Condition condition
    ) {
     ...
     */
    public long count(String tenantId, Condition condition) {
